...
public class CPU
{
    ...
    public int Step() {
        ...
        switch (opcode) {
            ...
            case 0x01: BC = mmu.ReadROM16(PC); PC += 2;      break; //LD BC,D16  3 12    ----
            case 0x02: mmu.WriteROM(BC, A);                break; //LD (BC),A  1 8     ----
            case 0x03: BC += 1;                             break; //INC BC     1 8     ----
            case 0x04: B = INC(B);                          break; //INC B      1 4     Z0H-
            case 0x05: B = DEC(B);                          break; //DEC B      1 4     Z1H-
            case 0x06: B = mmu.ReadROM(PC); PC += 1;       break; //LD B,D8    2 8     ----
            ...
            case 0x08: mmu.WriteROM16(mmu.ReadROM16(PC), SP); PC += 2; break; //LD (A16),SP 3 20   ----
            case 0x09: DAD(BC);                             break; //ADD HL,BC   1 8    -0HC
            case 0x0A: A = mmu.ReadROM(BC);                break; //LD A,(BC)   1 8    ----
            case 0x0B: BC -= 1;                             break; //DEC BC      1 8    ----
            case 0x0C: C = INC(C);                          break; //INC C       1 8    Z0H-
            case 0x0D: C = DEC(C);                          break; //DEC C       1 8    Z1H-
            case 0x0E: C = mmu.ReadROM(PC); PC += 1;       break; //LD C,D8     2 8    ----
            ...
            case 0x11: DE = mmu.ReadROM16(PC); PC += 2;      break; //LD DE,D16   3 12   ----
            case 0x12: mmu.WriteROM(DE, A);                break; //LD (DE),A   1 8    ----
            case 0x13: DE += 1;                             break; //INC DE      1 8    ----
            case 0x14: D = INC(D);                          break; //INC D       1 8    Z0H-
            case 0x15: D = DEC(D);                          break; //DEC D       1 8    Z1H-
            case 0x16: D = mmu.ReadROM(PC); PC += 1;       break; //LD D,D8     2 8    ----
            ...
            case 0x21: HL = mmu.ReadROM16(PC); PC += 2;      break; //LD HL,D16   3 12   ----
            case 0x22: mmu.WriteROM(HL++, A);              break; //LD (HL+),A  1 8    ----
            case 0x23: HL += 1;                             break; //INC HL      1 8    ----
            case 0x24: H = INC(H);                          break; //INC H       1 8    Z0H-
            case 0x25: H = DEC(H);                          break; //DEC H       1 8    Z1H-
            case 0x26: H = mmu.ReadROM(PC); PC += 1; ;     break; //LD H,D8     2 8    ----
            ...
            case 0x2A: A = mmu.ReadROM(HL++);                         break; //LD A (HL+) 1 8     ----
            case 0x2B: HL -= 1;                                        break; //DEC HL     1 4     ----
            case 0x2C: L = INC(L);                                     break; //INC L      1 4     Z0H-
            case 0x2D: L = DEC(L);                                     break; //DEC L      1 4     Z1H-
            case 0x2E: L = mmu.ReadROM(PC); PC += 1; ;                break; //LD L,D8    2 8     ----
            case 0x2F: A = (byte)~A; FlagN = true; FlagH = true;       break; //CPL	       1 4     -11-

            case 0x30: JR(!FlagC);                                break; //JR NC R8   2 12/8  ----
            case 0x31: SP = mmu.ReadROM16(PC); PC += 2; ;               break; //LD SP,D16  3 12    ----
            case 0x32: mmu.WriteROM(HL--, A);                         break; //LD (HL-),A 1 8     ----
            case 0x33: SP += 1;                                        break; //INC SP     1 8     ----
            case 0x34: mmu.WriteROM(HL, INC(mmu.ReadROM(HL)));       break; //INC (HL)   1 12    Z0H-
            case 0x35: mmu.WriteROM(HL, DEC(mmu.ReadROM(HL)));       break; //DEC (HL)   1 12    Z1H-
            case 0x36: mmu.WriteROM(HL, mmu.ReadROM(PC)); PC += 1;   break; //LD (HL),D8 2 12    ----
            ...
            case 0x46: B = mmu.ReadROM(HL);  break; //LD B,(HL)	1 8	   ----
            ...
            case 0x4E: C = mmu.ReadROM(HL);  break; //LD C,(HL)	1 8    ----
            ...
            case 0x56: D = mmu.ReadROM(HL);  break; //LD D,(HL)    1 8    ---- 
            ...
            case 0x5E: E = mmu.ReadROM(HL);  break; //LD E,(HL)    1 8    ----
            ...
            case 0x66: H = mmu.ReadROM(HL);  break; //LD H,(HL)    1 8    ----
            ...
            case 0x6E: L = mmu.ReadROM(HL);  break; //LD L,(HL)	1 8    ----
            case 0x6F: L = A;                 break; //LD L,A	    1 4    ----
                                                
            case 0x70: mmu.WriteROM(HL, B);  break; //LD (HL),B	1 8    ----
            case 0x71: mmu.WriteROM(HL, C);  break; //LD (HL),C	1 8	   ----
            case 0x72: mmu.WriteROM(HL, D);  break; //LD (HL),D	1 8	   ----
            case 0x73: mmu.WriteROM(HL, E);  break; //LD (HL),E	1 8	   ----
            case 0x74: mmu.WriteROM(HL, H);  break; //LD (HL),H	1 8	   ----
            case 0x75: mmu.WriteROM(HL, L);  break; //LD (HL),L	1 8	   ----
            case 0x76: HALT();             break; //HLT	        1 4    ----
            case 0x77: mmu.WriteROM(HL, A);  break; //LD (HL),A	1 8    ----
            ...
            case 0x7E: A = mmu.ReadROM(HL);  break; //LD A,(HL)    1 8    ----
            ...
            case 0x86: ADD(mmu.ReadROM(HL)); break; //ADD M	    1 8    Z0HC	
            ...
            case 0x8E: ADC(mmu.ReadROM(HL)); break; //ADC M	    1 8    Z0HC	
            ...
            case 0x96: SUB(mmu.ReadROM(HL)); break; //SUB M	    1 8    Z1HC
            ...
            case 0x9E: SBC(mmu.ReadROM(HL)); break; //SBC M	    1 8    Z1HC
            ...
            case 0xA6: AND(mmu.ReadROM(HL)); break; //AND M	    1 8    Z010
            ...
            case 0xAE: XOR(mmu.ReadROM(HL)); break; //XOR M	    1 8    Z000
            ...
            case 0xB6: OR(mmu.ReadROM(HL));  break; //OR M     	1 8    Z000
            ...
            case 0xBE: CP(mmu.ReadROM(HL));  break; //CP M     	1 8    Z1HC
            ...
            case 0xC6: ADD(mmu.ReadROM(PC)); PC += 1;  break; //ADD A,D8    2 8     Z0HC
            ...
            case 0xCB: PREFIX_CB(mmu.ReadROM(PC++));      break; //PREFIX CB OPCODE TABLE
            case 0xCC: CALL(FlagZ);                break; //CALL Z,A16  3 24/12 ----
            case 0xCD: CALL(true);                 break; //CALL A16    3 24    ----
            case 0xCE: ADC(mmu.ReadROM(PC)); PC += 1;  break; //ADC A,D8    2 8     ----
            ...
            case 0xD6: SUB(mmu.ReadROM(PC)); PC += 1;  break; //SUB D8      2 8     ----
            ...
            case 0xFA: A = mmu.ReadROM(mmu.ReadROM16(PC)); PC += 2;   break; //LD A,(A16)  3 16    ----
            case 0xFB: IMEEnabler = true;               break; //IE          1 4     ----
            //case 0xFC:                                break; //Illegal Opcode
            //case 0xFD:                                break; //Illegal Opcode
            case 0xFE: CP(mmu.ReadROM(PC)); PC += 1;   break; //CP D8       2 8     Z1HC
    ...
    private void PREFIX_CB(byte opcode) {
        switch (opcode) {
            ...
            case 0x06: mmu.WriteROM(HL, RLC(mmu.ReadROM(HL)));    break; //RLC (HL) 2   8   Z00C
            ...
            case 0x0E: mmu.WriteROM(HL, RRC(mmu.ReadROM(HL)));    break; //RRC (HL) 2   8   Z00C
            ...
            case 0x16: mmu.WriteROM(HL, RL(mmu.ReadROM(HL)));     break; //RL (HL)  2   8   Z00C
            ...
            case 0x1E: mmu.WriteROM(HL, RR(mmu.ReadROM(HL)));     break; //RR (HL)  2   8   Z00C
            ...
            case 0x26: mmu.WriteROM(HL, SLA(mmu.ReadROM(HL)));    break; //SLA (HL) 2   8   Z00C
            ...
            case 0x2E: mmu.WriteROM(HL, SRA(mmu.ReadROM(HL)));    break; //SRA (HL) 2   8   Z00C
            ...
            case 0x36: mmu.WriteROM(HL, SWAP(mmu.ReadROM(HL)));   break; //SWAP (HL) 2   8   Z00C
            ...
            case 0x3E: mmu.WriteROM(HL, SRL(mmu.ReadROM(HL)));    break; //SRL (HL) 2   8   Z000
            ...
            case 0x46: BIT(0x1, mmu.ReadROM(HL));                  break; //BIT (HL) 2   8   Z01-
            ...
            case 0x4E: BIT(0x2, mmu.ReadROM(HL));                 break; //BIT (HL) 2   8   Z01-
            ...
            case 0x56: BIT(0x4, mmu.ReadROM(HL));                 break; //BIT (HL) 2   8   Z01-
            ...
            case 0x5E: BIT(0x8, mmu.ReadROM(HL));                 break; //BIT (HL) 2   8   Z01-
            ...
            case 0x66: BIT(0x10, mmu.ReadROM(HL));                break; //BIT (HL) 2   8   Z01-
            ...
            case 0x6E: BIT(0x20, mmu.ReadROM(HL));                break; //BIT (HL) 2   8   Z01-
            ...
            case 0x76: BIT(0x40, mmu.ReadROM(HL));                break; //BIT (HL) 2   8   Z01-
            ...
            case 0x7E: BIT(0x80, mmu.ReadROM(HL));                break; //BIT (HL) 2   8   Z01-
            ...
            case 0x86: mmu.WriteROM(HL, RES(0x1, mmu.ReadROM(HL))); break; //RES (HL) 2   8   ----
            ...
            case 0x8E: mmu.WriteROM(HL, RES(0x2, mmu.ReadROM(HL))); break; //RES (HL) 2   8   ----
            ...
            case 0x96: mmu.WriteROM(HL, RES(0x4, mmu.ReadROM(HL))); break; //RES (HL) 2   8   ----
            ...
            case 0x9E: mmu.WriteROM(HL, RES(0x8, mmu.ReadROM(HL))); break; //RES (HL) 2   8   ----
            ...
            case 0xA6: mmu.WriteROM(HL, RES(0x10, mmu.ReadROM(HL))); break; //RES (HL) 2   8   ----
            ...
            case 0xAE: mmu.WriteROM(HL, RES(0x20, mmu.ReadROM(HL))); break; //RES (HL) 2   8   ----
            ...
            case 0xB6: mmu.WriteROM(HL, RES(0x40, mmu.ReadROM(HL))); break; //RES (HL) 2   8   ----
            ...
            case 0xBE: mmu.WriteROM(HL, RES(0x80, mmu.ReadROM(HL))); break; //RES (HL) 2   8   ----
            ...
            case 0xC6: mmu.WriteROM(HL, SET(0x1, mmu.ReadROM(HL))); break; //SET (HL) 2   8   ----
            ...
            case 0xCE: mmu.WriteROM(HL, SET(0x2, mmu.ReadROM(HL))); break; //SET (HL) 2   8   ----
            ...
            case 0xD6: mmu.WriteROM(HL, SET(0x4, mmu.ReadROM(HL))); break; //SET (HL) 2   8   ----
            ...
            case 0xDE: mmu.WriteROM(HL, SET(0x8, mmu.ReadROM(HL))); break; //SET (HL) 2   8   ----
            ...
            case 0xE6: mmu.WriteROM(HL, SET(0x10, mmu.ReadROM(HL))); break; //SET (HL) 2   8   ----
            ...
            case 0xEE: mmu.WriteROM(HL, SET(0x20, mmu.ReadROM(HL))); break; //SET (HL) 2   8   ----
            ...
            case 0xF6: mmu.WriteROM(HL, SET(0x40, mmu.ReadROM(HL))); break; //SET (HL) 2   8   ----
            ...
            case 0xFE: mmu.WriteROM(HL, SET(0x80, mmu.ReadROM(HL))); break; //SET (HL) 2   8   ----
    ...
    private ushort POP() {
        ushort ret = mmu.ReadROM16(SP);
        SP += 2;
        //byte l = mmu.ReadROM(++SP);
        //byte h = mmu.ReadROM(++SP);
        //ushort ret = (ushort)(h << 8 | l);
        //Console.WriteLine("stack POP = " + ret.ToString("x4") + " SP = " + SP.ToString("x4") + " reading: " + mmu.ReadROM16(SP).ToString("x4") + "ret = " /*+ ((ushort)(h << 8 | l)).ToString("x4")*/);
